    ...
    <div>
        ...
        <div class="relative mt-2 rounded-md shadow-sm">
            ...
            >
                ...
                <template #caret>
                    <span class="absolute inset-y-1 right-0 flex items-center rounded-r-md bg-white">
                ...
                <template #tag="{ option, handleTagRemove, disabled }">
                    <span class="relative inline-flex items-center px-2 py-1 font-normal bg-indigo-100 text-indigo-600 text-xs rounded-md">
                        {{ trackBy && option[trackBy] ? option[trackBy] : option }}

                        <button @click="(event) => handleTagRemove(option, event)" type="button" class="font-bold ml-1 -mr-1.5 inline-flex flex-shrink-0 items-center justify-center rounded-md hover:bg-indigo-200 focus:outline-none">
                            <span class="sr-only">Dismiss</span>
                            <XMarkIcon class="h-4 w-4 p-0.5 text-indigo-700" />
                        </button>
                    </span>
                </template>
    ...
    props: {
        ...
        mode: {
            ...
            validator: function (value) {
                return ['tags', 'multiple'].includes(value);
            },
        },
        error: {
            type: String,
        ...
        selectClasses() {
            return {
                container: `text-gray-900 mx-auto w-full flex items-center justify-end rounded-md ring-1 ring-inset focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6 min-h-[36px] ${
                    this.error ? 'ring-red-300 focus:ring-red-500 pr-9' : 'ring-gray-300 focus:ring-indigo-600 pr-3'
                }`,
                ...
                option: "flex items-center justify-start box-border text-left cursor-pointer block px-4 py-2 text-sm",
